...
import { ArrowLeft, FileText, Package, Send, XCircle } from "lucide-react";
import { BACKEND_URL } from "@/app/utility"; // Fix path as needed
...
export default function InitialRequestDetails() {
  ...
  useEffect(() => {
    const fetchDetails = async () => {
      try {
        const response = await axios.get(`${BACKEND_URL}/qt/${slug}`);
  ...
  if (isLoading) return <div className="p-20 text-center text-[#929090]">Loading...</div>;
  if (!data) return null;

  const totalAmount = data.items.reduce((sum: number, i: any) => sum + (Number(i.amount) || 0), 0);

  return (
    <div className="min-h-screen bg-[#F2F2F2] font-sans">
      <div className="mx-auto max-w-[1000px] px-6 py-10">
        <button onClick={() => router.back()} className="mb-6 inline-flex items-center gap-2 text-[13.5px] font-semibold text-[#929090] hover:text-[#111110]">
        ...
        <div className="mb-6 flex flex-wrap items-end justify-between gap-6 rounded-[14px] border border-black/[0.06] bg-white p-6 shadow-sm">
          <div>
            <div className="mb-2 flex items-center gap-2.5">
              ...
              <h1 className="text-[20px] font-bold text-[#111110]">{data.title}</h1>
            </div>
            <p className="text-[14px] text-[#929090]">Req #{data.id} · {data.department}</p>
          </div>
          <div className="text-right">
            <p className="text-[11px] font-semibold uppercase tracking-[0.06em] text-[#929090]">Est. Budget</p>
            <p className="font-mono text-[24px] font-bold text-[#111110]">₹{totalAmount.toLocaleString('en-IN')}</p>
          </div>
        </div>
              ...
              {data.items.map((item: any, i: number) => (
                <div key={item.id} className="rounded-[10px] border border-black/5 bg-[#F9FAFB] p-4">
                  <div className="mb-2 flex justify-between">
                    <span className="font-mono text-[11px] font-semibold text-[#929090]">ITEM {i + 1}</span>
                    <span className="font-mono text-[14px] font-bold text-[#111110]">₹{item.amount}</span>
                  </div>
                  <h4 className="text-[14px] font-bold">{item.name}</h4>
                  <p className="mt-1 text-[12.5px] text-[#4C433F]">{item.description}</p>
                </div>
              ))}
            </div>
          </div>

...
export default function QuotationsPage() {
  ...
  const [vendorResponse, setVendorResponse] = useState<number>(0);
  const [approvedQuotation, setApprovedQuotation] = useState<number>(0);

  useEffect(() => {
    const loadQuotations = async () => {
      setIsLoading(true);
      const data = await fetchQuotations();
      const qRes = await fetchResponses();

      const sortedById = [...data].sort((a, b) => b.id - a.id);
      if (data) setQuotations(sortedById);

      const calculateStatus = () => {
        const activeCount = data.filter(
          (data) => data.status != "DELIVERED",
        ).length;
        setActiveRequest(activeCount);

        const vendorCount = qRes.length;
        setVendorResponse(vendorCount);

        const approvedCount = data.filter(
          (q) => q.status === "APPROVED" || q.status == "DELIVERED",
        ).length;

        setApprovedQuotation(approvedCount);
      };
      calculateStatus();
      setIsLoading(false);
    };
  ...
  const renderStatusBadge = (status: Quotation["status"]) => {
    ...
    return (
      <span
        className={`inline-flex items-center rounded-full border px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.04em] ${styles[status] || styles.PENDING}`}
      >
      ...
      <main className="flex-1 px-[clamp(20px,4vw,40px)] py-[clamp(24px,4vw,40px)] transition-[margin-left] duration-300 ease-[cubic-bezier(0.22,1,0.36,1)] max-md:ml-[68px]">
        ...
        <div className="mb-6 grid grid-cols-1 gap-3 md:grid-cols-3">
          {[
            ...
            <div
              key={i}
              className="rounded-xl border border-black/[0.06] bg-white p-5 pb-4 shadow-sm hover:shadow-md transition-shadow"
            >
              ...
              <div
                className="mb-1 font-sans text-[28px] font-bold tracking-[-0.04em]"
                style={{ color: s.color }}
              >
                {s.val}
              </div>
                    ...
                    >
                      <td className="px-4 py-3.5">
                        <div className="flex flex-col">
                          ...
                          <span className="mt-0.5 font-mono text-[11px] text-[#929090]">
                            #{qt.id} · {qt.delivery_period}
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-3.5">
                      ...
                      <td className="px-4 py-3.5">
                        <span className="text-[13px] text-[#4C433F]">
                          {new Date(qt.submission_deadline).toLocaleDateString(
                            "en-GB",
